...
- 2026-06-01 — Seam: SQLite signal test and clean package verifier failed while adding `flatmachine_signal`; root cause: FlatMachines renders boolean template handoffs as `"True"` strings, and registry-resolved `@memgrafter/flatmachines@4.0.1` SQLite backend is not portable in the verifier runtime; canonical fix: match the wait-for example condition to rendered output and keep registry smoke coverage to safe memory signal send while documenting SQLite as runtime-dependent; validation: `npx vitest run tests/unit/opencode-plugin.test.ts` and `npm run verify:opencode-plugin`.
